...
let pp_opt pp fmt opt =
  match opt with
  | None -> ()
  | Some u -> F.fprintf fmt "%a" pp u

(* pretty printing constructor for lists *)
let pp_list sep pp fmt xx =
  match xx with
  | [] -> ()
  | _ -> F.pp_print_list ~pp_sep:(fun _ _ -> F.fprintf fmt sep) pp fmt xx

(* simple pattern of pp_list (base = "", sep = ",") *)
let pp_list_comma pp fmt xx = pp_list "," pp fmt xx

let pp_list_commaspace pp fmt xx = pp_list ", " pp fmt xx

let pp_list_newline pp fmt xx = pp_list "\n" pp fmt xx

let pp_list_none pp fmt xx = pp_list "" pp fmt xx
